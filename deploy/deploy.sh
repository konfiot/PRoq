...
echo "Copying files"
scp -r -i id_rsa -o StrictHostKeyChecking --exclude system.img --exclude arch.img --exclude arch.img.bz2 --exclude kernel-qemu --exclude id_rsa  --exclude id_rsa_sourceforge=no -P 5555 ./ root@localhost:./

echo "Installing"
ssh root@localhost -o StrictHostKeyChecking=no -p 5555 -i id_rsa "export EDITOR=cat ; yes | pacman -R heirloom-mailx mailx ; yes | pacman -Syu lighttpd python sox libid3tag libmad ffmpeg lame python2-pygame php-cgi php yaourt python2-pip ; echo 'n\ny\n' | yaourt -S svox-pico-git ; yes | pip-2.7 install icalendar ; systemctl enable lighttpd ; mkdir /etc/lighttpd/conf.d ; echo 'server.modules += ( \"mod_fastcgi\" ) index-file.names += ( \"index.php\" ) fastcgi.server = ( \".php\" => ((                    \"bin-path\" => \"/usr/bin/php-cgi\",                    \"socket\" => \"/tmp/php.socket\",                    \"max-procs\" => 2,                    \"bin-environment\" => (                      \"PHP_FCGI_CHILDREN\" => \"16\",                      \"PHP_FCGI_MAX_REQUESTS\" => \"10000\"                    ),                    \"bin-copy-environment\" => (                   \"PATH\", \"SHELL\", \"USER\"                    ),                    \"broken-scriptfilename\" => \"enable\"                )))' >> /etc/lighttpd/conf.d/fastcgi.conf ; echo 'include \"conf.d/fastcgi.conf\"' >> /etc/lighttpd/lighttpd.conf ; mv ~/WebConfiguration/* /srv/http/ ; rm .ssh/authorized_keys .ssh/known_hosts ; sed -i 's/sda/mmcblk0p/' /etc/fstab ; reboot"
