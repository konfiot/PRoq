...
echo "Installing"
ssh root@localhost -o StrictHostKeyChecking=no -p 5555 -i id_rsa "export EDITOR=cat ; yes | pacman -R heirloom-mailx ; yes | pacman -Syu lighttpd libsndfile python mpd libid3tag libmad ffmpeg lame python2-pygame php-cgi php yaourt python2-pip alsa-libs alsa-firmware ; echo 'n\ny' | yaourt -S svox-pico-git ; yes | pip-2.7 install icalendar python-mpd2 ; systemctl enable lighttpd mpd ; mkdir /etc/lighttpd/conf.d ; mkdir /var/lib/mpd/music ; touch /var/lib/mpd/mpd.db ; chown -R mpd:mpd  /var/lib/mpd/ ; echo 'music_directory "/var/lib/mpd/music"' >> /etc/mpd.conf ; echo 'server.modules += ( \"mod_fastcgi\" ) index-file.names += ( \"index.php\" ) fastcgi.server = ( \".php\" => ((                    \"bin-path\" => \"/usr/bin/php-cgi\",                    \"socket\" => \"/tmp/php.socket\",                    \"max-procs\" => 2,                    \"bin-environment\" => (                      \"PHP_FCGI_CHILDREN\" => \"16\",                      \"PHP_FCGI_MAX_REQUESTS\" => \"10000\"                    ),                    \"bin-copy-environment\" => (                   \"PATH\", \"SHELL\", \"USER\"                    ),                    \"broken-scriptfilename\" => \"enable\"                )))' >> /etc/lighttpd/conf.d/fastcgi.conf ; echo 'include \"conf.d/fastcgi.conf\"' >> /etc/lighttpd/lighttpd.conf ; mv ~/WebConfiguration/* /srv/http/ ; mkdir /srv/http/conf/ ; ln -s /srv/http/conf/ ~/PRoq/ ; rm .ssh/authorized_keys .ssh/known_hosts ; sed -i 's/sda/mmcblk0p/' /etc/fstab ; reboot"
